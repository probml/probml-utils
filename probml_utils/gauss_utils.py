    ...
    while no_conv and iteration < max_iter:
        ...
        for i in range(nr):
            if set(O[i,]) != set(
                ...
                Mui = Mu[np.ix_(M_i)] + (
                    S[np.ix_(M_i, O_i)]
                    @ np.linalg.pinv(S[np.ix_(O_i, O_i)] + e)
                    @ (X_tilde[i, np.ix_(O_i)].T - Mu[np.ix_(O_i)])
                )  # Expected stats for mean
                Vi = (
                    S[np.ix_(M_i, M_i)]
                    - S[np.ix_(M_i, O_i)] @ np.linalg.inv(S[np.ix_(O_i, O_i)] + e) @ S[np.ix_(M_i, O_i)].T
                )  # Expected stats for sigma
                Mui = Mui.reshape(-1, 1)
